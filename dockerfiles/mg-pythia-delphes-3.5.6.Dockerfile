...
# Tauola-1.1.8
ARG URL=https://tauolapp.web.cern.ch/resources/TAUOLA.1.1.9/TAUOLA.1.1.9-LHC.tar.gz
RUN mkdir ${TMP_DIR} && \
    cd ${TMP_DIR} && \
    wget ${URL} -O- | tar xz && \
    cd TAUOLA && \
    CXX=g++ ./configure --without-hepmc3 --with-hepmc=${INSTALL_DIR} --with-lhapdf=${INSTALL_DIR} --prefix=${INSTALL_DIR} && \
    make -j10 && \
    make install && \
    rm -r ${TMP_DIR}
